...
public class FaceLoginService
{
    ...
    public QRCode getQRCode() throws Exception {
        ...
        //2.生成二维码(url地址)
        String file = qrCodeUtil.createQRCode(url + "?code=" + code);
        //3.存入当前二维码状态（存入redis）
        FaceLoginResult result = new FaceLoginResult("-1");
        redisTemplate
                .boundValueOps(getCacheKey(code))
                .set(result, 10, TimeUnit.MINUTES);//状态对象，失效时间，单位
    ...
    public FaceLoginResult checkQRCode(String code) {
        ...
        return (FaceLoginResult) redisTemplate
                .opsForValue()
                .get(key);
    }
    ...
    public String loginByFace(String code, MultipartFile attachment) throws Exception {
        ...
        redisTemplate
                .boundValueOps(getCacheKey(code))
                .set(result, 10, TimeUnit.MINUTES);
        return userId;
    }
