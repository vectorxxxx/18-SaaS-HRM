...
public class LoginFilter extends ZuulFilter
{
	...
	/**
	 * String类型的返回值 定义过滤器类型的 pre     : 在执行路由请求之前执行 routing : 在路由请求是调用 post    : 在routing和error过滤器之后执行 error   : 处理请求出现异常的时候执行
	 */
	@Override
	...
	/**
	 * int类型的返回值 定义过滤器的优先级 : 数字越小,优先级越高
	 */
	@Override
	...
	/**
	 * run方法 : 过滤器中负责的具体业务逻辑 使用过滤器进行jwt的鉴权
	 */
	@Override
